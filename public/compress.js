...
var ImageResizer=function(opts){
    ...
    var innerTools={
        ...
        getImgFromDataSource:function(datasource,dataSourceType,callback){
			console.log("处理数据源",dataSourceType);
            var _me=this;
            var img1=new Image();
        ...
        getResizeSizeFromImg:function(img,f){
			console.log("getResizeSizeFromImg",$(img));
			image=$(img);
            ...
            if(_img_info.w <= settings.maxWidth && _img_info.h <= settings.maxHeight){
				console.log("getResizeSizeFromImg result",settings.maxWidth);
				console.log("getResizeSizeFromImg result",_img_info);
                return _img_info;
            }
        ...
        var canvas = document.createElement("canvas");
            canvas.width=theW;
            canvas.height=theH;
			console.log("_size_by_mh",theW,theH);
			console.log("sourceWidth",theW,theH);
    ...
    (function(){
        innerTools.getImgFromDataSource(settings.dataSource,settings.dataSourceType,function(_tmp_img){
            ...
            //--计算尺寸。
           innerTools.getResizeSizeFromImg(__tmpImg,function(_limitSizeInfo){
				 var _img_info={
			...
			//innerTools.drawToCanvas(__tmpImg,200,400,200,400,function(base64str,canvas)
			{  
